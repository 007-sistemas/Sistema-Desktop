...
export const BiometricCapture: React.FC<BiometricCaptureProps> = ({
  ...
  useEffect(() => {
    const initBiometry = async () => {
      try {
        setIsLoading(true);
        console.log('[BiometricCapture] Iniciando componente com SDK nativo...');
        
        // Aguarda o SDK ficar disponível (ex: scripts carregando a partir de index.html)
        const sdkReady = await biometryService.waitForSdkReady(5000);
        if (!sdkReady) {
          console.warn('[BiometricCapture] SDK não ficou pronto em 5s. Tentando novamente em 2s...');
          setMessage('Aguardando inicialização do SDK do leitor...');
          // Tenta novamente depois de um curto delay
          setTimeout(() => {
            initBiometry().catch(() => {});
          }, 2000);
          return;
        ...
        console.log('[BiometricCapture] ✅ SDK pronto — comunicação com o processo principal estabelecida.');

        // Configurar event listeners com retry automático
        biometryService.setListener({
          ...
          onDeviceDisconnected: (event: any) => {
            console.log('[BiometricCapture] ⚠️ Device desconectado');
            setMessage('Dispositivo desconectado, tentando reconectar...');
            setIsConnected(false);
            setIsCapturing(false);
            // Tentar recarregar devices e reconectar
        ...
        setError(`❌ Erro na inicialização: ${errorMsg}`);
        setIsConnected(false);
      } finally {
        setIsLoading(false);
      }
  ...
  const attemptDeviceRecovery = async () => {
    if (retryCount >= 5) {
      setError('❌ Falha ao reconectar após 5 tentativas. Reinicie o programa ou verifique o leitor.');
      return;
    }

    const nextRetryCount = retryCount + 1;
    setRetryCount(nextRetryCount);
    const delayMs = Math.min(2000 * nextRetryCount, 10000); // Backoff exponencial até 10s

    console.log(`[BiometricCapture] Tentando recuperar dispositivo (${nextRetryCount}/5) em ${delayMs}ms...`);
    ...
    retryTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        console.warn('[BiometricCapture] Falha na recuperação, tentando novamente...', err);
        attemptDeviceRecovery();
      }
    }, delayMs);
  };
  ...
  const loadDevices = async () => {
    try {
      ...
      if (deviceList.length === 0) {
        ...
        setRetryCount(0); // Reset retry ao sucesso
      }
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      console.error('[BiometricCapture] Erro ao listar leitores:', errorMsg);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2" title={isConnected ? 'Leitor conectado e pronto.' : 'Leitor desconectado ou não encontrado.'}>
