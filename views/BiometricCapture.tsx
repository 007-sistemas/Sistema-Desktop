...
export const BiometricCapture: React.FC = () => {
  ...
  const handleStartCapture = async () => {
      ...
      try {
          // **IMPORTANTE**: Configurar listeners ANTES de iniciar aquisição
          // Senão eventos ficarão perdidos
          biometryService.setListener({
              onDeviceConnected: () => {
                console.log('[BiometricCapture] onDeviceConnected');
                setMessage("Leitor conectado");
              },
              onDeviceDisconnected: () => { 
                console.log('[BiometricCapture] onDeviceDisconnected');
                setMessage("Leitor desconectado"); 
                setIsCapturing(false); 
              },
              onQualityReported: (e: any) => {
                console.log('[BiometricCapture] onQualityReported:', e?.quality);
                setQuality(`Qualidade: ${e.quality}`);
              },
              onSamplesAcquired: (s: any) => {
                  console.log('[BiometricCapture] onSamplesAcquired');
              ...
              onErrorOccurred: (e: any) => {
                  console.error('[BiometricCapture] onErrorOccurred:', e);
                  const msg = e?.message || "Ocorreu um erro no leitor.";
                  setError(msg);
                  setIsCapturing(false);
              }
          });

          console.log('[BiometricCapture] Iniciando aquisição com device:', selectedDevice);
          await biometryService.startAcquisition(currentFormat, selectedDevice);
          console.log('[BiometricCapture] Aquisição iniciada com sucesso');
          setIsCapturing(true);
          setMessage("Coloque o dedo no sensor...");
      } catch (e: any) {
          console.error('[BiometricCapture] Erro ao iniciar captura:', e);
          const msg = e?.message || "Falha ao iniciar captura. Verifique se o serviço está rodando.";
          setError(msg);
          setIsCapturing(false);
      }
  };
  ...
  const handleStopCapture = async () => {
      try {
          console.log('[BiometricCapture] Parando captura');
